# DistRDF version of the RDataFrame benchmark df103
...
# - Encapsulated the declaration of user code in a DistRDF init function
# - Added #ifndef to prevent double definition in init function
...
#ifndef df103_NanoAODHiggsAnalysis
#define df103_NanoAODHiggsAnalysis
...
def run(path, npartitions):
    ...
    df_sig_4l = RDataFrame("Events", [os.path.join(path, "SMHiggsToZZTo4L.root")]*10, npartitions=npartitions)
    ...
    # tutorial.
    df_bkg_4mu = RDataFrame("Events", [os.path.join(path, "ZZTo4mu.root")]*10, npartitions=npartitions)
    df_bkg_4el = RDataFrame("Events", [os.path.join(path, "ZZTo4e.root")]*10, npartitions=npartitions)

    # CMS data taken in 2012 (11.6 fb^-1 integrated luminosity)
    df_data_doublemu = RDataFrame("Events", [os.path.join(path, f) for f in ["Run2012B_DoubleMuParked.root", "Run2012C_DoubleMuParked.root"]]*10, npartitions=npartitions)
    df_data_doubleel = RDataFrame("Events", [os.path.join(path, f) for f in ["Run2012B_DoubleElectron.root", "Run2012C_DoubleElectron.root"]]*10, npartitions=npartitions)
