...
df103_opt_cxxnojit   = df103_opt.Filter('test_type == "cpp_lambdas"')

datasets = [ 'df_sig_4l', 'df_bkg_4mu', 'df_data_doublemu', 'df_bkg_4el', 'df_data_doubleel', 'all_datasets'  ]
time_types = [ 'event_loop', 'jit', 'compile_macro', 'compilation', 'getvalue_trigger' ]

for dataset in datasets:
    for df,times_dict in (df103_opt_cxxdistrdf,df103_times_cxxdistrdf), (df103_opt_cxxnojit,df103_times_cxxnojit):
...
print("Compilation time CxxNoJIT, all datasets: {} s".format(df103_times_cxxnojit['all_datasets']['compilation']))

# Set 'rest' to be "time that is not jitting nor running event loop nor running CompileMacro (if it applies)"
for times_dict in df103_times_cxxdistrdf, df103_times_cxxnojit:
    for dataset in datasets:
...
for time_type, color in zip(time_types,colors):
    ...
    for dataset in datasets:
        ...
        t.append(df103_times_cxxnojit[dataset][time_type])
    t.append(0)
